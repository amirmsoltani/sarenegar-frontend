...
export const useMedicineUsageCountsModal = () => {
  ...
  const onSubmit = () => {
    const dayCounts = getValues("medicine_usage_counts_placeholder");
    setValue("medicine_usage_counts", dayCounts, { shouldValidate: formState.isSubmitted });
    _ref.current?.close();
  };
