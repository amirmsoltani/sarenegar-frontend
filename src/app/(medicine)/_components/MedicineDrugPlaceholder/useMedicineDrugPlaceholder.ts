...
export const useMedicineDrugPlaceholder = () => {
  ...
  if (startTime && startTime.value && drug && medicineUsageCounts) {
    const reminders = DateService.createReminders(
      `${startTime!.value!.hour.value}:${startTime!.value!.minute.value}`,
      +medicineUsageCounts!.value!,
    );
  ...
  } else if (drug && drugCounts) {
    useText = `${drugCounts.value} ${mapForm2Name(drug.form.name)}`;
  }
